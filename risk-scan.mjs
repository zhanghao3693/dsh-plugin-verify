 ...
 *
 * 2026-09-26：medium 细项在站点端展示层已合并为三类（读写/删除本地文件、
 * 读取剪贴板或密钥环境变量、可执行外部命令），见 deepseek-harness-hub
 * lib/risk-scan-labels.ts 的 REASON_MERGE_GROUPS。这里同步收拢描述口径；
 * decide() push 的逐条 reasons 不变 —— 数据层保留细粒度事实，合并只发生在展示。
 */
const LEVEL_MAP = {
  high: ["插件市场安装脚本（安装即执行代码）", "动态拼接的命令执行（参数含运行时变量）"],
  medium: ["可执行外部命令", "读写/删除本地文件", "读取剪贴板或密钥环境变量"],
  low: ["仅网络请求 + 读取自身元数据"],
};
